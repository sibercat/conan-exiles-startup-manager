...
from core.default_config import DEFAULT_CONFIG, DEFAULT_DISCORD_CONFIG
    ...
    def _get_message_control(self) -> Dict[str, bool]:
        ...
        try:
            if 'server' in self.config and 'message_control' in self.config['server']:
                return {**DEFAULT_CONFIG['server']['message_control'], 
                       **self.config['server']['message_control']}
            return DEFAULT_CONFIG['server']['message_control'].copy()
        except Exception as e:
    ...
    def _get_message_type(self, message: str) -> Optional[str]:
        ...
        if "[WARNING]" in message:
            if "network" in message.lower():
                ...
                return "zombie_detected_notification"
    ...
    def send_message(self, message: str) -> bool:
        ...
        try:
            ...
            payload = {
                ...
                'username': self.config.get('server', {}).get('name', DEFAULT_CONFIG['server']['name'])
            }
