...
class LogHandler(FileSystemEventHandler):
   def __init__(self, log_dir, discord_manager=None, logger=None, ports=None, startup_delay=0, messages=None):
   ...
   def on_created(self, event):
       ...
       if event.src_path.endswith("ConanSandbox.log"):
           ...
           if self.discord:
               self.discord.send_message(self.messages.get("loading"))
               ...
               with open(event.src_path, 'r', encoding='utf-8') as f:
                   ...
                   for line in reversed(lines):
                       if self.LOAD_COMPLETE_STRING in line and not self.load_complete:
                           ...
                           self.server_exiting = False
                           if self.discord:
                               self.discord.send_message(self.messages.get("ready"))
                           break
                           ...
                           if self.discord:
                               self.discord.send_message(self.messages.get("shutdown"))
                           break
           except Exception as e:
               self.logger.error(f"Error reading log file: {e}")
               print(f"Error reading log file: {e}")

def monitor_server(log_dir, discord_manager, logger, ports, startup_delay, messages):
   """Main server monitoring function"""
   event_handler = LogHandler(log_dir, discord_manager, logger, ports, startup_delay, messages)
   observer = Observer()
   ...
   try:
       ...
       allow_ports(ports)
       if discord_manager:
           discord_manager.send_message(messages.get("monitor_stop"))
...
def main():
   ...
   if discord_manager and discord_manager.is_enabled():
       discord_manager.send_message(messages.get("startup"))
   
   logger.info("Starting server monitoring...")
   print("\nStarting server monitoring...")
   
   try:
       monitor_server(log_dir, discord_manager, logger, ports, startup_delay, messages)
   except KeyboardInterrupt:
